...
Proof.
  intros P ex s H. destruct H as (s' & Ps' & DS). unfold denote in DS.

  assert (Hexec : exists n, [nRepeat C n] ex |=> (s', s)). {
    dependent induction DS.
    - (* EStar0: 0 iterations *)
      exists 0%nat. constructor.
    - specialize (IHDS1 C s2 s' Ps' eq_refl eq_refl). invs IHDS1.
      exists (S x). simpl. constructor. exists s2. now split.
    - (* EStarEr: error happens (possibly after 0 ok-iterations; this constructor is “immediate”) *)
      exists 1%nat. simpl. apply ESeqOk. exists s'. split. constructor. assumption.
  }
...
Proof.
  ...
  - intros s Ps. destruct (IH _ Ps) as (s' & Qs' & DS). invs DS.
    ...
    econstructor; eassumption.
  - assert (forall n sigma, P n sigma -> exists sigma', P O sigma' /\ [C**] ok |=> (sigma', sigma)). {
      induction n; intros.
        exists sigma. split. assumption. constructor.
      specialize (H0 _ _ H1). destruct H0 as (s' & Ps' & DS).
      specialize (IHn s' Ps'). destruct IHn as (sigma' & P0 & DRS).
      exists sigma'. split. assumption. econstructor. apply DRS. apply DS.
    } intros s Ps. destruct Ps. eapply H1. eassumption.
...
Proof.
  ...
  - destruct ex.
    -- eapply Consequence. constructor.
        intros s Ps. apply Ps.
        intros s HQ. destruct (DS s HQ) as (s' & _ & H). invs H.
    -- eapply Consequence with (Q := P). apply UnitOk.
        intros s Ps. apply Ps.
        intros s HQ. destruct (DS s HQ) as (s' & HP & H). invs H. assumption.
  - destruct ex.
    -- eapply Consequence. constructor. intros s Ps. apply Ps.
       intros s Qs. destruct (DS s Qs) as (s' & HP & H). invs H.
    -- eapply Consequence with (Q := (fun s => exists i', P (s[i := i']) /\
                              s i = exp (s[i := i']))). constructor. easy.
       intros s Qs. destruct (DS s Qs) as (s' & HP & H). invs H.
       eexists. split. rewrite update_shadow. now rewrite state_upd_eq.
       now rewrite update_eq, update_shadow, state_upd_eq.
  - destruct ex.
    -- eapply Consequence. constructor. intros s Ps. apply Ps.
       intros s Qs. destruct (DS s Qs) as (s' & HP & H). invs H.
    -- eapply Consequence with (Q := (fun s => exists x', P (s[i := x']))).
         constructor. easy.
       intros s Qs. destruct (DS s Qs) as (s' & HP & H). invs H.
       eexists. rewrite update_shadow. now rewrite state_upd_eq.
 - destruct ex.
   -- set (Mid := fun s2 => exists s1, P s1 /\ [C1] ok |=> (s1, s2)).
      ...
      assert (DS_C1_er : {{P}} denote C1 er {{Qsc}}).
        intros s3 (HQsc & s & Ps & DS'). now exists s.
      ...
      eapply Consequence with
       (P  := fun s => P s \/ P s)
       (Q  := fun s => Qsc s \/ Qnm s).
      exact D_or. now intros s [].
      intros s HQ. destruct (DS s HQ) as (s1 & HP & Hseq). invs Hseq.
     + left. split. assumption. exists s1. now split.
     + right. split. assumption. destruct H2 as (s2 & Hc1ok & Hc2er).
       exists s2. split. exists s1. now split. assumption.
   -- set (Mid := fun s2 => exists s1, P s1 /\ [C1] ok |=> (s1, s2)).
      ...
      assert (DS_C2_ok : {{Mid}} denote C2 ok {{Qnm}}).
        intros s2 (Qs2 & s & Ps & DS'). now exists s.
      ...
      eapply Consequence with (Q := fun s => Qnm s).
        apply D_nm. now intro.
      intros s HQ. destruct (DS s HQ) as (s1 & HP & Hseq). split. assumption.
      invs Hseq. destruct H2 as (s2 & DS1 & DS2). exists s2. split.
      + exists s1. now split.
      + assumption.
  - set (Mid1 := fun s2 => exists s1, P s1 /\ [C1] ex |=> (s1, s2)).
    ...
    -- assert (DS_C1 : {{P}} denote <{ C1 }> ex {{Mid1}}).
         intros s (s' & Ps' & DS'). now exists s'.
       assert (DS_C2 : {{P}} denote <{ C2 }> ex {{Mid2}}).
         intros s (s' & Ps' & DS'). now exists s'.

       specialize (IHC1 P Mid1 ex DS_C1).
       specialize (IHC2 P Mid2 ex DS_C2).
       assert (A1 : P, [C1 <+> C2] ex, Mid1) by now constructor.
       assert (A2 : P, [C1 <+> C2] ex, Mid2) by now constructor.
 ...
 - set (p := fun n => ((ds_post (denote (nRepeat C n) ok) P))). 
   ...
   assert (Hok : P, [C **] ok, post). {
        ...
        assert (DS_C: {{p n}} denote C ok {{p (S n)}}). {
          ...
          destruct H2 as (s2 & DS1 & DS2). destruct n.
            simpl in DS1. invs DS1. unfold p. simpl. exists s2.
            split. exists s2. split. assumption. constructor. assumption.
  ...
  - destruct ex.
    ...
    -- eapply Consequence with (Q := (fun _ => False)). apply ErrorOk. intros s Ps. apply Ps.
       intros s Qs. destruct (DS s Qs) as (s' & Ps' & DS'). invs DS'.
  - destruct ex.
    -- eapply Consequence with (Q := (fun s => False)). apply AssumeEr. intros s Ps. apply Ps.
       intros s Qs. destruct (DS s Qs) as (s' & Ps' & DS'). invs DS'.
...
Proof.
  intros. unfold interpret_spec.
  split; intro.
  - now apply soundness.
  - now apply completeness.
Qed.